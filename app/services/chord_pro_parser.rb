...
class ChordProParser
  ...
  def substitute_chords_riffs(html_output)
    html_output.gsub!(/\[Riff([\S\s]+?)\]/, '<kbd>Riff \1</kbd>')
    html_output.gsub!(/\[(\S+?)\]/, '<kbd>\1</kbd>')
  end

  def substitute_meta_data(html_output)
  ...
  def subsitute_sections(html_output)
    html_output.gsub!(/\{comment:([\S\s]+?)\}/,
                      '<span class="comment text-muted">\1</span>')
    html_output.gsub!(/\{c:([\S\s]+?)\}/,
                      '<span class="comment text-muted">\1</span>')
    html_output.gsub!(/\{chorus\}/,
                      '<span class="comment text-muted">Chorus</span>')

    html_output.gsub!(/\{start_of_verse\}([\S\s]+?)\{end_of_verse\}/,
                      '<p class="verse">\1</p>')
    html_output.gsub!(/\{sov\}([\S\s]+?)\{start_of_verse\}/,
                      '<p class="verse">\1</p>')

    html_output.gsub!(/\{soc\}([\S\s]+?)\{eoc\}/,
                      '<blockquote>\1</blockquote>')
    html_output.gsub!(/\{start_of_tab\}([\S\s]+?)\{end_of_tab\}/,
                      '<p class="tab">\1</p>')
  end
